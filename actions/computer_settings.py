...
def sleep_computer():
    if _OS == "Windows":
        try:
            ...
            subprocess.run(["powershell", "-Command", "Add-Type -Assembly System.Windows.Forms; [System.Windows.Forms.Application]::SetSuspendState('Suspend', $false, $false)"], capture_output=True, **_WIN_HIDE)
    elif _OS == "Darwin":
        subprocess.run(["pmset", "sleepnow"], capture_output=True)
    else:
        subprocess.run(["systemctl", "suspend"], capture_output=True)

def sign_out_computer():
    if _OS == "Windows":
        subprocess.run(["shutdown", "/l"], capture_output=True, **_WIN_HIDE)
    elif _OS == "Darwin":
        subprocess.run(["osascript", "-e", 'tell application "System Events" to log out'], capture_output=True)
    else:
        try:
            import os
            subprocess.run(["loginctl", "terminate-user", os.getlogin()], capture_output=True)
        except Exception:
            pass
...
ACTION_MAP: dict[str, callable] = {
    ...
    "stop_song":           stop_media,
    "stop_music":          stop_media,
    "pause_music":         play_pause_media,
    "resume_music":        play_pause_media,
    "play_music":          play_pause_media,
    "turn_music_off":      stop_media,
    "close_app":           close_app,
    ...
    "reboot_pc":           restart_computer,
    "shutdown":            shutdown_computer,
    "shutdown_pc":         shutdown_computer,
    "turn_off_pc":         shutdown_computer,
    "sleep_pc":            sleep_computer,
    "sleep_computer":      sleep_computer,
    "sign_out":            sign_out_computer,
    "logout":              sign_out_computer,
    "log_out":             sign_out_computer,
}
