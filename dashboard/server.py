...
def _log_remote_step(step_num: int, title: str, status: str, details: str = ""):
    ts = time.strftime("%H:%M:%S")
    status_str = "[SUCCESS]" if status == "OK" else f"[FAILED:{status}]"
    msg = f"[REMOTE LOG {step_num}/11] [{ts}] {title} | Status: {status_str}"
    if details:
        msg += f" | {details}"
    try:
        print(msg)
...
class DashboardServer:
    ...
    def __init__(self):
        ...
        _log_remote_step(5, "LAN IP Detection", "OK", f"Reachable LAN IPv4: {self._ip}")
    ...
    def new_key(self, expiry_secs: int = 600) -> str:
        ...
        _log_remote_step(1, "QR Code Generation", "OK", f"Pairing Code: {key} (Expires in {expiry_secs}s)")
        return key
    ...
    def get_url(self) -> str:
        ...
        _log_remote_step(2, "Target URL Generation", "OK", f"URL: {url}")
        return url
...
<p>Connecting to ULTRON…</p>
        ...
        async def pairing_info(refresh: bool = False):
            """Returns current or new pairing key, LAN QR target URL, and device status."""
            now = time.time()
            self._pending_keys = {k: v for k, v in self._pending_keys.items() if v > now}
            if refresh or not self._pending_keys:
                code = self.new_key(600)
            else:
                code = next(iter(self._pending_keys.keys()))
            
            lan_url = f"http://{self._ip}:{PORT}/auto-login?key={code}"
            connected_devices = len(self._clients)
            status = "CONNECTED" if connected_devices > 0 else ("PAIRED" if self._device_sessions else "WAITING FOR PHONE")
            return JSONResponse({
                "ok": True,
                "code": code,
                "url": lan_url,
        ...
        async def revoke_devices(req: Request):
            ...
            asyncio.create_task(self.broadcast({"type": "sys", "text": "All paired remote devices revoked."}))
            return JSONResponse({"ok": True, "revoked": count})

        @app.get("/api/screen_frame")
        async def get_screen_frame():
            """Returns a compressed JPEG screen capture frame."""
            try:
                import io
                import mss
                ...
                try:
                    ...
                    return JSONResponse({"error": f"{e} | {inner_e}"}, status_code=500)

        @app.get("/api/settings")
        async def get_settings():
            """Returns full configuration dictionary."""
            try:
                from ui import _read_full_config
                cfg = await asyncio.to_thread(_read_full_config)
        ...
        async def save_settings(req: Request):
            """Saves updated settings to persistent config file."""
            try:
                data = await req.json()
                from ui import _read_full_config, API_FILE, CONFIG_DIR, BASE_DIR
                cfg = await asyncio.to_thread(_read_full_config) or {}
                
                fields = [
                    "assistant_name", "user_name", "greeting", "personality",
                    "wake_word", "accent_color", "theme", "default_mode",
                    "auto_start", "morning_brief_enabled"
                ]
                for f in fields:
                    if f in data:
                        cfg[f] = data[f]
                ...
                if "auto_start" in data and sys.platform == "win32":
                    def _set_win_autostart(enable: bool):
                        try:
                            import winreg
                            ...
                            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0, winreg.KEY_SET_VALUE) as key:
                                if enable:
                                    ...
                                    winreg.SetValueEx(key, "UltronOS", 0, winreg.REG_SZ, cmd)
                                else:
                                    try:
                                        winreg.DeleteValue(key, "UltronOS")
                                    except Exception:
                                        pass
                        except Exception as ex:
                            print(f"[ULTRON] Windows Auto-Start update error: {ex}")
                    await asyncio.to_thread(_set_win_autostart, bool(data["auto_start"]))
        ...
        async def screen_frame_ep(req: Request):
            ...
            try:
                ...
                return Response(content=img_bytes, media_type=mime or "image/jpeg")
            except Exception as e:
        ...
        async def ws_ep(websocket: WebSocket, token: str = ""):
            ...
            if not is_local and (not tok or tok not in self._tokens):
                _log_remote_step(6, "WebSocket Handshake", "AUTH_DENIED", f"Client IP: {client_ip}")
                await websocket.close(code=4001)
                return
            if not tok:
                tok = secrets.token_urlsafe(32)
            ...
            try:
                while True:
                    ...
                    if data.get("type") == "command":
                        ...
                        if t:
                            _log_remote_step(8, "Phone -> Desktop Message", "OK", f"Received Command: '{t}'")
                        ...
                        if b64_data:
                            _log_remote_step(10, "File Upload", "OK", f"Image attachment received ({mime})")
    ...
    async def serve(self) -> None:
        if not _DEPS_OK:
            _log_remote_step(3, "Local Server Startup", "MISSING_DEPS", "fastapi/uvicorn not installed")
        ...
        _log_remote_step(3, "Local Server Startup", "OK", f"Host: 0.0.0.0:{PORT}")
        _log_remote_step(4, "Port Availability Check", "OK", f"Port {PORT} bound successfully")
